"""
Sinhala TTS Fine-Tuning with Spark-TTS

Fine-tune Spark-TTS (0.5B Qwen2-based) on Sinhala voice recordings.

Usage:
    python sinhala_tts.py                          # use data/sentences.tsv
    python sinhala_tts.py --tsv <path>             # use a custom TSV
    python sinhala_tts.py --resume sinhala_tts_lora  # continue from existing adapter
"""
...
def main():
    parser = argparse.ArgumentParser(description="Sinhala TTS fine-tuning with Spark-TTS")
    ...
    parser.add_argument("--resume", default=None, metavar="ADAPTER_DIR",
                        help="Continue training from an existing LoRA adapter (e.g. sinhala_tts_lora)")
    args = parser.parse_args()

    print("=" * 70)
    print("SPARK-TTS SINHALA TTS FINE-TUNING")
    print(f"Dataset: {args.tsv}")
    if args.resume:
        print(f"Resume:  continuing from {args.resume}")
    print("=" * 70)

    print("\n[Step 1] Loading Spark-TTS model...")
    ...
    trainer = TTSSFTTrainer(
        ...
        args=TTSSFTConfig(
            output_dir="outputs_sinhala_tts",
    ...
    print("Saved to sinhala_tts_lora/")

    print("\n" + "=" * 70)
    print("Done! Spark-TTS Sinhala TTS fine-tuning complete.")
    print("=" * 70)
    print("\nTo synthesize Sinhala speech:")
    print("  uv run python synthesize.py 'ආයුබෝවන්'")
